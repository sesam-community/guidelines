...
echo "Docker image is set to'${secrets.DOCKER_USERNAME}/${_REPO_NAME}:${_DOCKER_REPO_TAG}'"


#build docker image
docker build --label Commit="${GITHUB_SHA}" --label BuildNumber="${GITHUB_RUN_NUMBER}" --label RepoSlug="${_REPO}" -t ${secrets.DOCKER_USERNAME}/${_REPO_NAME}:${_DOCKER_REPO_TAG} .

#push to dockerhub if tagged or pushed to master
if  [ "${GITHUB_REF:0:10}" = "refs/tags/" ] || [ "${GITHUB_REF}" = "refs/heads/master" ]
then
  docker push ${secrets.DOCKER_USERNAME}/${_REPO_NAME}:${_DOCKER_REPO_TAG}
  docker logout
else
